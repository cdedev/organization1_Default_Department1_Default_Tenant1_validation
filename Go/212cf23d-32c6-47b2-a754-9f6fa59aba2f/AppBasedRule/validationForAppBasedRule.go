...
import (
	...
	"strconv"
	"strings"
	parser "organization1_Default_Department1_Default_Tenant1_validation/Go/212cf23d-32c6-47b2-a754-9f6fa59aba2f/AppBasedRule/parser"
...
func (g pluginFunction) CallValidation(input []map[string]string,runIDs []string) []interface{}{
	ruleErrorDetails := Validate(input,runIDs)
	return ruleErrorDetails
}
...
type ValidationListener struct {
	*parser.BaseAppBasedRuleListener
}
...
func getGrammarFile() string{
	...
	file := `grammar AppBasedRule;

expression :  glucose glucose_acosh EOF ;

glucose : GLUCOSE ;

glucose_acosh : GLUCOSE_ACOSH ;

GLUCOSE :SPLITKEY  OWNKEY [0-9]* ;

GLUCOSE_ACOSH :SPLITKEY  OWNKEY OWNKEY [0-9]* ;
...
func Validate(inputData []map[string]string,runIDs []string) []interface{}{
	var ruleErrorDetails []interface{}
	//Ownkey
	ownKey:="$~$"
	var keys []string
	if len(inputData) > 0 {
		for k,_ := range inputData[0]{
			keys = append(keys,k)
		}
	}
	...
	if len(keySortToGrammar)==0{
		if len(keys) == 0{
			for index := 0 ; index < len(inputData) ; index++ {
				ruleErrorDetails = append(ruleErrorDetails,"")
			}
			return ruleErrorDetails
		}else{
			for index := 0 ; index < len(inputData) ; index++ {
				ruleErrorDetails = append(ruleErrorDetails,ErrorInfo{})
			}
			return ruleErrorDetails
		}
	...
	for index,input := range inputData {
		...
		ErrorListMap.Delete(runID)
		ErrorHandleMap.Delete(runID)
	}
...
func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
	...
	lexer:=parser.NewAppBasedRuleLexer(in)
	if outerMap, ok := ErrorHandleMap.Load(runID); ok {
		if value, canConvert := outerMap.(bool); canConvert {
	...
	parser:=parser.NewAppBasedRuleParser(stream)
	if outerMap, ok := ErrorHandleMap.Load(runID); ok {
		if value, canConvert := outerMap.(bool); canConvert {
...
func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
	...
	errorInfo.RuleName = "AppBasedRule"
